...
local Button = Tab:CreateButton({
   Name = "Slap Fight(key is slapcondo404)",
   Callback = function()
...
local VisualizerToggle = Instance.new("TextButton")

-- UI Setup
ScreenGui.Parent = game.CoreGui
MainFrame.Name = "DolphinGui"
MainFrame.Parent = ScreenGui
...
Title.Parent = MainFrame
Title.Size = UDim2.new(1, 0, 0, 30)
Title.Text = "Dolphin Slapper"
Title.TextColor3 = Color3.new(1, 1, 1)
Title.BackgroundColor3 = Color3.fromRGB(50, 50, 50)

AuraToggle.Name = "AuraToggle"
AuraToggle.Parent = MainFrame
AuraToggle.Position = UDim2.new(0, 10, 0, 40)
AuraToggle.Size = UDim2.new(0, 180, 0, 40)
AuraToggle.Text = "Slap Aura: OFF"
AuraToggle.BackgroundColor3 = Color3.fromRGB(200, 50, 50)

RangeInput.Parent = MainFrame
RangeInput.Position = UDim2.new(0, 10, 0, 90)
RangeInput.Size = UDim2.new(0, 180, 0, 40)
RangeInput.PlaceholderText = "Aura Range (e.g. 20)"
RangeInput.Text = "20"

VisualizerToggle.Parent = MainFrame
VisualizerToggle.Position = UDim2.new(0, 10, 0, 140)
VisualizerToggle.Size = UDim2.new(0, 180, 0, 40)
VisualizerToggle.Text = "Visualizer: OFF"

SlapAllBtn.Parent = MainFrame
SlapAllBtn.Position = UDim2.new(0, 10, 0, 190)
SlapAllBtn.Size = UDim2.new(0, 180, 0, 40)
SlapAllBtn.Text = "Slap All (One-Shot)"
...
-- Remotes from your source
local SlapRemote = rs:WaitForChild("gloves"):WaitForChild("Non"):WaitForChild("Slap")
local HitterRemote = rs:WaitForChild("gloves"):WaitForChild("Non"):WaitForChild("Hitter")
local HitRemote = rs:WaitForChild("Hit")

-- Visualizer Part Fix: Using Transparency instead of Visible
local Visualizer = Instance.new("Part")
Visualizer.Shape = Enum.PartType.Ball
Visualizer.Color = Color3.new(0, 1, 0)
Visualizer.Transparency = 1 -- Start invisible
Visualizer.CanCollide = false
Visualizer.Anchored = true
Visualizer.Material = Enum.Material.ForceField
Visualizer.Parent = workspace

-- Function to execute slap logic
local function SlapTarget(target)
    if target and target:FindFirstChild("HumanoidRootPart") then
        SlapRemote:FireServer("Active", "Normal")
        local arm = lp.Character:FindFirstChild("Right Arm") or lp.Character:FindFirstChild("Left Arm")
        SlapRemote:FireServer("Swing", "Normal", arm, 1)
        HitRemote:FireServer(target, 7, 10, 2, 0.15, false, true)
        HitterRemote:FireServer(target, "Normal")
    end
end
...
AuraToggle.MouseButton1Click:Connect(function()
    ...
    AuraToggle.Text = AuraEnabled and "Slap Aura: ON" or "Slap Aura: OFF"
    AuraToggle.BackgroundColor3 = AuraEnabled and Color3.fromRGB(50, 200, 50) or Color3.fromRGB(200, 50, 50)
end)

VisualizerToggle.MouseButton1Click:Connect(function()
    VisualizerEnabled = not VisualizerEnabled
    -- Fix here: Toggle transparency between 0.8 (visible) and 1 (invisible)
    Visualizer.Transparency = VisualizerEnabled and 0.8 or 1
    VisualizerToggle.Text = VisualizerEnabled and "Visualizer: ON" or "Visualizer: OFF"
end)

RangeInput:GetPropertyChangedSignal("Text"):Connect(function()
    AuraRange = tonumber(RangeInput.Text) or 20
end)

SlapAllBtn.MouseButton1Click:Connect(function()
    for _, v in pairs(game.Players:GetPlayers()) do
        if v ~= lp and v.Character and v.Character:FindFirstChild("HumanoidRootPart") then
            SlapTarget(v.Character)
        end
    end
end)
...
-- Main Loop
runService.RenderStepped:Connect(function()
    if VisualizerEnabled and lp.Character and lp.Character:FindFirstChild("HumanoidRootPart") then
        Visualizer.Position = lp.Character.HumanoidRootPart.Position
        Visualizer.Size = Vector3.new(AuraRange * 2, AuraRange * 2, AuraRange * 2)
    end

    if AuraEnabled and lp.Character and lp.Character:FindFirstChild("HumanoidRootPart") then
        for _, v in pairs(game.Players:GetPlayers()) do
            if v ~= lp and v.Character and v.Character:FindFirstChild("HumanoidRootPart") then
